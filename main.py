...
def encode_vid_data():
    cap=cv2.VideoCapture("/content/cover_video.mp4")
    vidcap = cv2.VideoCapture("/content/cover_video.mp4")
    fourcc = cv2.VideoWriter_fourcc(*'XVID')
    frame_width = int(vidcap.get(3))
    frame_height = int(vidcap.get(4))

    size = (frame_width, frame_height)
    out = cv2.VideoWriter('/content/stego_video.mp4',fourcc, 25.0, size)
    max_frame=0;
    while(cap.isOpened()):
        ret, frame = cap.read()
        if ret == False:
...
def decode_vid_data(frame_):
    cap = cv2.VideoCapture('/content/stego_video.mp4')
    max_frame=0;
    while(cap.isOpened()):
        ret, frame = cap.read()
        if ret == False:
